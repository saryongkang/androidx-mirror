...
class VerticalTextBuilderTest {
    ...
    @Test
    fun text_withRubyMap_attachesRubyAtEachMatch() {
        val result = buildVerticalText(density) {
            text("吾輩は猫である。吾輩はねこだ。", mapOf("吾輩" to "わがはい"))
        }
    ...
    @Test
    fun tateChuYoko_wrapsTextInTextCombineUprightSpan() {
        val result = buildVerticalText(density) { tateChuYoko("25") }

        val spans =
            result.getSpans(0, result.length, TextOrientationSpan.TextCombineUpright::class.java)
        assertThat(spans).hasLength(1)
        assertThat(result.getSpanStart(spans[0])).isEqualTo(0)
        assertThat(result.getSpanEnd(spans[0])).isEqualTo(2)
    }
    ...
    @Test
    fun ruby_wrapsBlockRangeWithAnnotation() {
        val result = buildVerticalText(density) {
            text("前")
            ruby("くりすます") { text("クリスマス") }
            text("後")
        }
    ...
    @Test
    fun withFontShear_wrapsBlockRangeInFontShearSpan() {
        val result = buildVerticalText(density) { withFontShear { text("斜体") } }
    ...
    fun withEmphasis_wrapsBlockRangeInEmphasisSpan() {
        val result = buildVerticalText(density) {
            withEmphasis(style = EmphasisStyle.Sesame) { text("強調") }
        }
    ...
    fun nestedBuildVerticalText_appendedAsCharSequence_preservesInnerSpans() {
        ...
        val result = buildVerticalText(density) {
            ruby(buildVerticalText(density) { sideways("inline") }) { text("外側") }
        }
    ...
    @Test
    fun density_exposedToNestedBuilds() {
        // density must be reachable inside the builder lambda for nested buildVerticalText calls.
        val expected = Density(density = 3.5f, fontScale = 1.25f)
        var observed: Density? = null

        buildVerticalText(expected) { observed = density }
    ...
    @Test
    fun composableOverload_usesLocalDensity() = runComposeUiTest {
        var result: android.text.Spanned? = null
