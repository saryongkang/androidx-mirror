 ...
 * nesting blocks such as [VerticalTextScope.ruby] or [VerticalTextScope.withStyle].
 */
...
public interface VerticalTextScope {
    /**
     * The [Density] used to resolve sp-based [TextUnit] values inside [withStyle]. Exposed so
     * that nested [buildVerticalText] blocks can reuse the same density.
     */
    public val density: Density

    /**
     * Appends [text] and wraps it in a [TextOrientationSpan.Sideways] span: every glyph is
     * rotated 90° clockwise regardless of its script.
     */
    ...
    /**
     * Appends [text] and wraps it in a [TextOrientationSpan.TextCombineUpright] span: all of the
     * appended characters are combined horizontally and laid out within a single vertical slot
     * (tate-chu-yoko).
     */
    public fun tateChuYoko(text: CharSequence)

    /**
     * Runs [block] and wraps the range it appends with a [RubySpan] annotating it with [ruby].
     *
     * @return the value returned by [block].
     */
    public fun <R : Any> ruby(ruby: CharSequence, block: VerticalTextScope.() -> R): R

    /**
     * Appends [text]. For each `(key, reading)` in [rubyMap], every occurrence of `key` inside
     * the just-appended [text] is annotated with a [RubySpan] carrying `reading`.
     */
    public fun text(text: CharSequence, rubyMap: Map<String, String> = emptyMap())

    /**
     * Runs [block] and wraps the range it appends with a [MetricAffectingSpan] applying the
     * given [fontSize], [textColor], and/or [backgroundColor] to the paint used for that range.
     *
    ...
    /**
     * Runs [block] and wraps the range it appends with a [FontShearSpan] producing a slanted
     * (italic-like) presentation suited to Japanese typography.
     *
     * @return the value returned by [block].
     */
    public fun <R : Any> withFontShear(
...
internal class VerticalTextBuilder(override val density: Density) : VerticalTextScope {
    ...
    override fun tateChuYoko(text: CharSequence): Unit =
        withSpan(TextOrientationSpan.TextCombineUpright()) { this.text(text) }

    override fun <R : Any> ruby(ruby: CharSequence, block: VerticalTextScope.() -> R): R =
    ...
    override fun <R : Any> withFontShear(fontShear: Float, block: VerticalTextScope.() -> R): R =
        withSpan(FontShearSpan(fontShear), block)
...
/**
 * Builds a [Spanned] representing vertical text by running [builder] against a
 * [VerticalTextScope]. The resulting [Spanned] is an immutable snapshot that can be passed to
 * [VerticalText].
 *
 ...
 */
public fun buildVerticalText(
    density: Density,
    builder: VerticalTextScope.() -> Unit,
): Spanned = VerticalTextBuilder(density).apply(builder).build()

/**
 * Composable-context convenience for [buildVerticalText]. Uses [LocalDensity] to resolve
 * sp-based [TextUnit] values. For non-composable callers or nested builds, use the overload
 * that takes an explicit [Density].
 *
 * @param builder the DSL block describing the content.
 */
@Composable
public fun buildVerticalText(
    builder: VerticalTextScope.() -> Unit
): Spanned = buildVerticalText(LocalDensity.current, builder)
